...
const Landing = () => {
    ...
    return (
        <div className={"h-full text-white bg-gradient-to-b from-[#1e1e20] to-[#0e0e11] flex flex-col justify-between relative font-Inter"}>

            <nav className={'flex justify-between h-14 p-8 relative z-20'}>
                <ModeToggle/>
                <div className={'grid grid-cols-2 gap-x-4 font-medium text-sm'}>
                    <Button onClick={ ()=> navigate("/sign-in")} className={"rounded-lg"} size={"sm"} variant={"transparent"} >
                        Se connecter  <span className={"ml-3"}>🪄</span>
                    </Button>
                    <Button onClick={ ()=> navigate("/sign-up")}  className={"rounded-lg"} size={"sm"} variant={"default"} >
                ...
                <div className={"relative"}>
                    <div className={"flex flex-col items-center md:p-0 p-8"}>
                        ...
                        <div className={'grid grid-cols-2 gap-x-4 mt-8'}>
                            <Button onClick={()=>navigate("/sign-in")} className={"text-sm rounded-full font-semibold"} size={"lg"} variant={"transparent"} >
                                Se connecter  <span className={"ml-3"}>🪄</span>
                            </Button>
                            <Button onClick={()=>navigate("/sign-up")} className={"text-sm rounded-full font-semibold"} size={"lg"} variant={"transparent"} >
                                S'inscrire  <span className={"ml-3"}>🔮</span>
                            </Button>
                        </div>
                    </div>
